  ...
  toc: {
    initAgent: {
      ...
      howTo: [
        ...
        "Verify: `echo \"$SUPABASE_SECRETE_KEY\" | head -c 12` should print a non-empty prefix. `echo \"$NEXT_PUBLIC_SUPABASE_URL\"` should print `https://sdrqhejvvmbolqzfujej.supabase.co`.",
        "If either is empty: the writeAgent setup block was skipped or your shell lost the source. Run `source ~/.guildos.env` once more. If that still fails, your spawn was created without the credentials block — escalate immediately with `housekeeping.escalate` (`detail.reason='spawn missing GuildOS credentials block'`, `detail.unblock_path='Guildmaster respawns this agent via cursor.writeAgent (the only supported path)'`). Do NOT try to work around this gap by uploading to your project's Supabase — that's the documented 2026-04-26 ptglab failure mode.",
        "",
        "**Step 1: Pull project repo + clone GuildOS**",
        "On your project repo: pull latest on current branch. If `git pull origin main` fails, run `git remote show origin | grep HEAD` to find the default branch and pull that instead. Merge conflicts → trigger setNewAgent.",
        "Clone GuildOS: `git clone https://github.com/boster00/GuildOS.git ~/guildos` (or `cd ~/guildos && git pull origin main` if already cloned). Then `cd ~/guildos && npm install`.",
        "This gives you: global instructions, skill books, weapons, env-cred templates.",
        "",
        "**Step 2: Persist creds into the GuildOS .env.local**",
        "The repo's `.env.local` is gitignored and missing on fresh clones. Mirror your shell vars into it so node scripts that read `--env-file=.env.local` work without extra exports:",
        "```bash",
        "cat > ~/guildos/.env.local <<EOF",
