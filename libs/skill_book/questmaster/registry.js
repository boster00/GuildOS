...
`,
    ...
    closeQuest: {
      ...
      input: {
        quest_id: "string — closing quest UUID (alias questId)",
        asana_task_gid: "string — optional; aliases asana_task_id, task_id",
      },
      howTo: [
        "**Closing flow:**",
        "1. Read the quest description, inventory, and key comments",
        "2. Confirm the managerial summary (closeQuest builds one from the quest + recent comments)",
        "3. Ensure an Asana task GID is present (link in description/title/deliverables, or pass asana_task_gid)",
        "4. Run skillbook **questmaster** (or **questmaster_registry**), action **closeQuest** — posts summary, completes the Asana task, sets quest stage to **complete**",
        "5. If no task GID: escalate to Guildmaster to identify the right Asana task",
        "6. Success path records a quest comment: Quest closed. Summary archived to Asana.",
      ].join("\n"),
    },
  },
};
