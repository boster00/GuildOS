  ...
  toc: {
    audit: {
      ...
      howTo: [
        ...
        "MAGENTO + CF CACHE PATTERN (bosterbio.com confirmed 2026-04-25): A low HTML hit ratio on a Magento site behind CF is almost always a Magento-side issue, NOT a CF configuration issue. Magento assigns session cookies (frontend, PHPSESSID, form_key) to every visitor. CF uses these in the cache key, so each unique session = a unique cache miss even with override_origin. Anonymous curl tests will show HIT (no cookies); real-user aggregate analytics will show mostly miss/none. Fix is via SSH to the Magento/nginx config to strip or scope session cookies before they reach CF's cache key. Do NOT contact the hosting company (JetRails) for this — it is not a hosting issue.",
