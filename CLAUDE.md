...
- **`questPurrview.approve`** — purrview → review. Requires per-item Cat verdicts, all `pass`, **and each verdict must carry `judge_origin` from `openai_images.judge` (or `claude-multimodal-read` for the local final gate)**. Cat must invoke the judge per item before posting verdicts — the layer-attribution finding (5f9b4c0) showed Cat alone has 0/2 catch rate; the judge is mandatory, not optional. The gate writes `items.purrview_check` (T2) per item via `libs/quest/items.writeReview` + the APPROVE lockphrase: "this quest now meets the criteria for review".
- **`questPurrview.bounce`** — purrview → execute. Requires per-item verdicts with ≥1 `fail` + a non-empty `reason` + each verdict carrying `judge_origin` (same mandate as `approve`). Writes `items.purrview_check` (T2) per item + BOUNCE lockphrase: "this quest has been bounced back to execute". Worker fixes the listed `item_keys` and resubmits.
